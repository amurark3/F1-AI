...
def _summarize_circuit_edition(race_data: dict, edition_year: int) -> dict | None:
    ...
    median_first_stop = int(round(median(first_stop_laps)))
    ...
    finishing_compound = (
        finishing_counter.most_common(1)[0][0] if finishing_counter else "HARD"
    )
    ...
    return {
        ...
        "first_stop_p25": int(round(_percentile(first_stop_laps, 0.25) or median_first_stop)),
        "first_stop_p75": int(round(_percentile(first_stop_laps, 0.75) or median_first_stop)),
