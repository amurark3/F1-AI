...
export default function TeamDetailPage() {
  ...
  return (
    <div>
      ...
      <MetricRow>
        <MetricCard label="Position" value={team.position ? `P${team.position}` : "No rank"} sub="Constructor table" icon={Trophy} color={team.color} />
        ...
        <MetricCard label="Drivers" value={String(team.drivers.length)} sub="Active roster" icon={Users} color="#00FF78" />
      </MetricRow>

      <WorkspaceSplit className="xl:[&>*:first-child]:basis-[58%] xl:[&>*:last-child]:flex-1">
        <Panel accent={team.color} className="p-5">
          <h2 className="text-xl font-black italic uppercase text-white mb-4" style={rcFont}>Standing Profile</h2>
          <div className="grid gap-2 sm:grid-cols-2">
            {Object.entries(team.standing_profile ?? team.pace_profile ?? {}).map(([label, value]) => (
              <div key={label} className="rounded-lg border border-white/6 bg-white/[0.025] p-3">
                ...
                <p className="mt-1 text-2xl font-black text-white" style={rcFont}>{formatPoints(value)}</p>
          ...
          <div className="mt-6 flex flex-col gap-4 sm:flex-row [&>*]:flex-1">
            <div>
              ...
              <div className="space-y-2">
                {team.strengths.map((item) => <StatusPill key={item} color={team.color}>{item}</StatusPill>)}
              </div>
            </div>
            <div>
              <p className="text-[10px] font-black uppercase tracking-[0.2em] text-neutral-600 mb-3">Pressure Points</p>
              <div className="space-y-2">
                {team.weaknesses.map((item) => <StatusPill key={item} color="#FFF200">{item}</StatusPill>)}
              </div>
            </div>
          </div>
        </Panel>
        ...
        <div className="space-y-5">
          <Panel className="p-5">
            <h2 className="text-xl font-black italic uppercase text-white mb-4" style={rcFont}>Teammate View</h2>
            <div className="space-y-2">
              {team.drivers.length > 0 ? team.drivers.map((driver) => (
                <div key={driver.driver} className="rounded-lg border border-white/6 bg-white/[0.025] p-3">
                  <div className="flex items-center justify-between gap-3">
                    ...
                    <span className="text-lg font-black text-neutral-300" style={rcFont}>{driver.points}</span>
                  </div>
                </div>
          ...
          <Panel className="p-5">
            <h2 className="text-xl font-black italic uppercase text-white mb-4" style={rcFont}>Data Boundaries</h2>
            <div className="space-y-2">
              {[
                ...
                <div key={item} className="flex items-center gap-3 rounded-lg border border-white/6 bg-white/[0.025] px-3 py-2">
