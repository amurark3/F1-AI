...
function PreRequestView({ scheduleReady, noUpcoming, upcomingRace, onRun }: {
  ...
  return (
    ...
    >
      ...
      {scheduleReady && !noUpcoming && upcomingRace && (
        <UpcomingRaceCTA raceName={upcomingRace.name} onRun={onRun} />
      )}
    </motion.div>
  );
}
...
const PredictionPanel = () => {
  ...
  const { data, isLoading, error: swrError, mutate } = useSWR<PredictionsResponse, Error>(
    ...
    <div className="relative">
      ...
      {!isLoading && (swrError || backendError) && !hasContent && (
        <PredictionErrorState
          message={firstNonBlank(backendError, swrError?.message)}
          onRetry={() => void mutate()}
          onBack={() => setRequested(false)}
        />
      )}
...
function PredictionErrorState({ message, onRetry, onBack }: {
  message?: string;
  onRetry: () => void;
  onBack: () => void;
}) {
...
function PredictionResults({ data, podium, grid, onReset }: {
