    ...
    <div>
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-sm">
          ...
          <tbody>
            {positions.map((p) => (
              <tr key={p.driver} className="border-b border-white/5 hover:bg-white/5 transition-colors">
                <td className="py-2 pr-4 text-neutral-400 font-mono text-xs">P{p.position}</td>
                <td className={`py-2 pr-4 font-mono font-bold ${p.position === 1 ? "text-white" : "text-neutral-300"}`}>
                  #{p.driver}
                </td>
                <td className="py-2 text-neutral-400 font-mono text-xs">{p.gap === "LEADER" ? "\u2014" : p.gap}</td>
              </tr>
            ))}
          </tbody>
        </table>
