...
import type { TeamsResponse } from "../teams/teamsModel";
...
/** Server-rendered payloads used to seed the intel board's SWR caches. */
export interface IntelSeed {
  year: number;
  teams: TeamsResponse | null;
  /** Intel for the default constructor, or null if there was none to prefetch. */
  intel: Intel | null;
  /** The slug `intel` belongs to, so a different selection is not seeded. */
  intelSlug: string | null;
}

export function IntelView({ seed }: { seed: IntelSeed }) {
  const { year } = seed;
  ...
  const { data, error, isLoading, mutate } = useSWR<Intel, Error>(
    ...
    fetcher,
    {
      revalidateOnFocus: false,
      dedupingInterval: 180000,
      // Only the constructor the server prefetched is seeded.
      ...seededWith(selectedTeam === seed.intelSlug ? seed.intel : null),
    },
  );
  const color = data?.team.color ?? "#00FF78";
  const pageLoading = awaitingFirstData(teamsLoading, teams.length > 0);
  ...
  return (
    <div>
      ...
      <IntelTeamsNotice
        show={!pageLoading && Boolean(teamsError || teamsData?.error || teams.length === 0)}
        message={teamsData?.error}
        onRetry={() => void reloadTeams()}
      />

      <IntelContent
        isLoading={awaitingFirstData(isLoading, data !== undefined)}
        error={error}
        data={data}
        color={color}
        onRetry={() => void mutate()}
...
function IntelContent({
  ...
  if (data) {
    return <IntelReport data={data} color={color} />;
  }
  return null;
}

function IntelSelector({
  teams,
  selectedTeam,
  onSelectTeam,
  color,
    ...
    <Panel className="p-5 mb-5">
      <div className="flex flex-col sm:flex-row sm:items-end gap-3">
        <label className="w-full sm:w-72">
          <span className="block text-[10px] font-black uppercase tracking-[0.18em] text-neutral-600 mb-2">
            Target team
          </span>
          <select
            value={selectedTeam}
            ...
            className="w-full rounded-lg bg-white/[0.04] border border-white/10 px-3 py-2 text-sm text-white outline-none disabled:text-neutral-500"
          >
        ...
        <StatusPill color={color}>{teamName ?? "Profile updating"}</StatusPill>
        {hasSource && <StatusPill color="#3671C6">Official standings</StatusPill>}
      </div>
    </Panel>
  );
}

function IntelTeamsNotice({ show, message, onRetry }: { show: boolean; message?: string | null; onRetry: () => void }) {
  if (!show) {
    return null;
  }
  return (
    <div className="mb-5">
      <InlineNotice title="Constructor Feed Unavailable" tone="error">
        {message ?? "The constructor standings API did not respond."}
        <button onClick={onRetry} className="ml-2 font-bold text-white underline decoration-white/30">
          Retry
        </button>
      </InlineNotice>
...
function IntelReport({ data, color }: { data: Intel; color: string }) {
  return (
    <>
      <MetricRow>
        ...
        <MetricCard
          ...
          value={String(data.upgrade_watch.length)}
          sub="Standings-derived facts"
          icon={Database}
          color="#3671C6"
        />
        <MetricCard
          label="Threats"
          value={String(data.threats.length)}
          sub="Championship pressure"
          icon={AlertTriangle}
          color="#E10600"
        />
        <MetricCard
          ...
          <div className="flex flex-col gap-4 sm:flex-row [&>*]:flex-1">
            <div>
              ...
              <div className="space-y-2">
                {data.team.strengths.map((item) => (
                  <StatusPill key={item} color={color}>
                    {item}
                  </StatusPill>
                ))}
              </div>
        ...
        <Panel className="p-5">
          ...
          <div className="space-y-2">
            {data.threats.map((item) => (
              <div
                key={item}
                className="rounded-lg border border-red-500/20 bg-red-500/[0.035] p-3 text-sm text-neutral-400"
              >
                {item}
              </div>
            ))}
        ...
        <Panel className="p-5">
          ...
          <div className="space-y-2">
            {data.opportunities.map((item) => (
              <div
                key={item}
                className="rounded-lg border border-[#00FF78]/20 bg-[#00FF78]/[0.035] p-3 text-sm text-neutral-400"
              >
                {item}
              </div>
            ))}
