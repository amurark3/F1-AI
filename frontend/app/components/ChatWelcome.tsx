...
const PROMPTS: Prompt[] = [
  {
    color: "#E10600",
    label: "Championship",
    text: "Brief me on the current title fight and the biggest points swing risk",
    icon: Trophy,
  },
  {
    color: "#3671C6",
    label: "Driver Delta",
    text: "Compare Norris vs Verstappen qualifying at Monza 2024",
    icon: BarChart3,
  },
  {
    color: "#FF8000",
    label: "Regulations",
    text: "What 2026 regulation changes matter most for race operations?",
    icon: FileSearch,
  },
  {
    color: "#27F4D2",
    label: "Technical",
    text: "Explain DRS in terms of attack, defense, and race strategy",
    icon: Gauge,
  },
  {
    color: "#00CC00",
    label: "Schedule",
    text: "When is the next race and what sessions should I watch?",
    icon: CalendarClock,
  },
    ...
    <div className="flex flex-col items-center justify-center h-full px-4 sm:px-6 pb-6 sm:pb-8">
      ...
      >
        ...
        <div className="inline-flex items-center gap-2 px-3 py-1 mb-5 rounded border text-[10px] font-black uppercase tracking-[0.2em] sector-best">
          <span className="h-[7px] w-[7px] rounded-full animate-glow-pulse" style={{ background: "#BE3AFF" }} />
          Public Pit Wall
        </div>
        ...
        <p className="text-neutral-500 text-sm sm:text-base max-w-md mx-auto leading-relaxed">
          Ask the race engineer for race context, driver comparisons, regulations, strategy language, and championship
          implications.
        </p>
      </motion.div>
        ...
        {PROMPTS.map(({ color, label, text, icon: Icon }) => (
          ...
          >
            ...
            <div className="px-4 pt-3 pb-4">
              ...
              <div className="text-[9px] font-black uppercase tracking-[0.18em] mb-2" style={{ color }}>
                {label}
              </div>
