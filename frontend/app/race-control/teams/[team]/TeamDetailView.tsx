...
export function TeamDetailView({ slug, year, initial }: { slug: string; year: number; initial: TeamDetailResponse | null }) {
  // Seeded when the server reached the backend, fetched here when it did not.
  const { data, isLoading } = useSWR<TeamDetailResponse>(
    `${API_BASE}/api/race-control/teams/${slug}/${year}`,
    fetcher,
    { revalidateOnFocus: false, dedupingInterval: 180000, ...seededWith(initial) },
  );
