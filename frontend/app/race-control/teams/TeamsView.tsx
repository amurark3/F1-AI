...
interface TeamsViewProps {
  year: number;
  /** Standings rendered on the server; null when the backend was unreachable. */
  initialTeams: TeamsResponse | null;
}

/**
 * Championship hub.
 *
 * Stays a client component because the refresh button drives SWR's `mutate`,
 * but it no longer *starts* empty: the server passes the standings it already
 * fetched as SWR's fallback, so the table is in the initial HTML and SWR
 * revalidates underneath it.
 */
export function TeamsView({ year, initialTeams }: TeamsViewProps) {
  const {
    data,
    error,
    isLoading,
  ...
  return (
    <div>
      ...
      <TeamsErrorNotice
        show={!pageLoading && Boolean(error || data?.error || teams.length === 0)}
          ...
          <div className="min-w-0">
            ...
            <p className="mt-1 text-sm text-neutral-400">
              ...
              {generatedAt && (
                <>
                  {" · refreshed "}
                  <LocalTime value={generatedAt} style="stampZone" />
                </>
              )}
            </p>
          </div>
